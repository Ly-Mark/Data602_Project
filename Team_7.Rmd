...
```{r}
#Hypothesis Test
# We will be conducting this test at .05 level of significance.
Ho <- "MuMaleBmi = MuFemaleBMI"
Ha <- "MuMaleBmi != MuFemaleBMI"

##First we Isolate the data into the desired age-ranges 
male2039 <- maleAgeBMI[maleAgeBMI$RIDAGEYR<=39,]
Female2039 <- femaleAgeBMI[femaleAgeBMI$RIDAGEYR<=39,]

male4059 <- maleAgeBMI[c(maleAgeBMI$RIDAGEYR>=40 &maleAgeBMI$RIDAGEY<=59),]
Female4059 <- femaleAgeBMI[c(femaleAgeBMI$RIDAGEYR>=40 & femaleAgeBMI$RIDAGEYR<=59 ),]

male6080 <- maleAgeBMI[c(maleAgeBMI$RIDAGEYR>=60 & maleAgeBMI$RIDAGEYR<=81),]
Female6080 <- femaleAgeBMI[c(femaleAgeBMI$RIDAGEYR>=60 & femaleAgeBMI$RIDAGEYR<=81),]

#Then we conduct the hypothesis test for the desired age-ranges
#First are males and females ages 20-39

t.test(Female2039$BMXBMI,male2039$BMXBMI)
Pvalue <- .09045
CI <- c(-0.07108265,0.97233594)
Conclusion <- "Since the Pvalue is greater than our level of significance we do not reject the null. This indicates that 
males and females BMI means are equal for this age group."

#Second test, Ages 40-59
t.test(Female4059$BMXBMI,male4059$BMXBMI)
pvalue <- '2.511e-05'
CI <- c(0.5474608,1.4976780)
Conclusion <- "Since the Pvalue is less than our level of significance we do reject the null. This indicates that 
males and females BMI means are not equal for this age group."

t.test(Female6080$BMXBMI,male6080$BMXBMI)
