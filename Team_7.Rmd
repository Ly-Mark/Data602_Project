...
3. We to get the count, mean and standard error for each of our BMI categories for each of our ethnicity. To accomplish this, we will create a function that takes advantage of *dplyr*'s forward pipe (*%>%*), *group_by*, and *summarize* commands
...
We will start by doing a hypothesis test on whether there is any statically significant difference in the BMI's of males and females across all age groups that are $\geq20$ years old. In our sample, we have a total of 4782 males and 4975 females across all age groups 
...
Null hypothesis states that there is no difference between population mean BMI's of males and females.

Alternative hypothesis states there is a difference between population mean BMI's of males and females.
...
From our t-test, our test statistic value was $t = -6.0054$. Since our test statistics does not appear within our range of critical values, we once again reject our null hypothesis, $H_o : \mu_{maleBMI} = \mu_{femaleBMI}$, which means that their is a difference between mean BMI's of males and females.
...
For the age ranges of 40 to 59 and 60+, we see that the P-value is low in both cases, $2.511e^{-5}$ and $5.205e^{-6}$, respectively. We would reject our null hypothesis, $H_o:  \mu_{maleBMI} = \mu_{femaleBMI}$ at the $\alpha=0.05$ level. This matches our initial exploration that looked at BMI's of males and females over all age groups and that there is a difference between mean BMI's.
...
For the 20 to 39 range, we see that our test statistic is -1.6936 which falls inside of our range of (-1.962 and 1.962). This means that we fall to reject our null hypothesis at the 5% level and our tests results are not strong enough to say that the mean BMI's between males and females are different at this range. Again, this matches our P-value conclusion that we determined previously. 
